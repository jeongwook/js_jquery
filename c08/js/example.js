$ (function() {                                             // When the DOM is ready
    ...
    var times;                                              // Declare local variable
    $.ajax({                                                 // Setup request
    ...
    function loadTimetable() {                              // Declare function
        $.getJSON('data/example.json')                      // Try to college JSON data
        .done( function(data){                              // If successful
            times = data;                                   // Store it in a variable
        }).fail( function() {                               //If a problem: show message
            $('#event').html('Sorry! We coudl not load the timetable at the moment');
        });
    }
    ...
    $('#content').on('click', '#event a', function(e) {     // User clicks on place
        ...
        var newContent = '';                                // To build timetable
        for (var i = 0; i < times[loc].length; i++){        // Look through sessions
        ...
        $('#sessions').html('<ul>' + newContent + '</u>');  // Display time
